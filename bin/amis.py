...
def get_model_name(name):
    if name == "esm1b":
        from efficient_evolution.bin.fb_model import FBModel
...
def soft_reconstruct(seq, model, alpha=1.0, offset=1):
    ...
    mutations = []
    true_probs = []
    for i in range(probs.shape[0] - 1):  # probs.shape: (seq_len, vocab_size)
        if i == 0:
            continue
        pos = i - offset
        wt_j = model.alphabet_.tok_to_idx[
            seq[pos]
        ]  # get the vocab_index of the residue (its mapping)
        wt_prob = probs[
            i, wt_j
        ]  # get the probability (output log) of this residue in this place
        for j in range(probs.shape[1]):
            mt = model.alphabet_.all_toks[j]  # get the mapped vocab of each index
            if mt in exclude or "<" in mt:
                continue
            if (
                j == wt_j
            ):  # wt_j is the vocab_index of the original residue (original sequence)
                true_probs.append(probs[i, j])
                # continue
            # mt_prob = probs[i, j] # i: actual_python_index of the residue, j: vocab_index
            # if mt_prob > alpha * wt_prob:
            #     mutations.append((pos, seq[pos], mt))
    true_probs_avg = sum(true_probs) / len(true_probs)
    return true_probs_avg
...
def sequence_prob(wt_seq, models, alpha=None):
    probs = {}
    for model in models:
        probs[model.name_] = soft_reconstruct(
            wt_seq,
            model,
            alpha=alpha,
        )
